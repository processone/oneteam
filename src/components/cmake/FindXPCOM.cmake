...
IF(XPCOM_GECKO_SDK)
    FIND_PROGRAM(XPCOM_XPIDL_BIN
                 NAMES xpidl host_xpidl
                 HINTS "${XPCOM_GECKO_SDK}/sdk/bin" "${XPCOM_GECKO_SDK}/host/bin")
    FIND_PROGRAM(XPCOM_XPT_LINK_BIN
                 NAMES xpt_link host_xpt_link
                 HINTS "${XPCOM_GECKO_SDK}/sdk/bin" "${XPCOM_GECKO_SDK}/host/bin")
    ...
    IF(CMAKE_SYSTEM_NAME MATCHES "Darwin")
        STRING(REGEX REPLACE "i[3-6]86" "x86" XPCOM_SYSTEM_PROCESSOR ${CMAKE_OSX_ARCHITECTURES})
    ELSE(CMAKE_SYSTEM_NAME MATCHES "Darwin")
        STRING(REGEX REPLACE "i[3-6]86" "x86" XPCOM_SYSTEM_PROCESSOR ${CMAKE_SYSTEM_PROCESSOR})
    ENDIF(CMAKE_SYSTEM_NAME MATCHES "Darwin")

    IF(WIN32)
        SET(XPCOM_ABI "WINNT_${XPCOM_SYSTEM_PROCESSOR}-msvc")
    ELSE(WIN32)
        SET(XPCOM_ABI "${CMAKE_SYSTEM_NAME}_${XPCOM_SYSTEM_PROCESSOR}-gcc3")
    ENDIF(WIN32)
    ...
    IF(CMAKE_SYSTEM_NAME MATCHES "Darwin")
        ...
        SET(XPCOM_LIBXUL ${XPCOM_GECKO_SDK}/lib/XUL)
    ELSE(CMAKE_SYSTEM_NAME MATCHES "Darwin")
        SET(XPCOM_LIBXUL xul)
    ENDIF(CMAKE_SYSTEM_NAME MATCHES "Darwin")

    IF(XPCOM_GECKO_VERSION LESS 200)
        SET(XPCOM_LIBRARIES xpcomglue_s xpcom nspr4 ${XPCOM_LIBXUL})
    ELSE(XPCOM_GECKO_VERSION LESS 200)
        SET(XPCOM_LIBRARIES xpcomglue_s_nomozalloc xpcom nspr4 ${XPCOM_LIBXUL})
    ...
    IF(UNIX)
        ...
        IF(CMAKE_SYSTEM_NAME MATCHES "Linux")
            SET(XPCOM_C_FLAGS "${XPCOM_C_FLAGS} -fdata-sections -ffunction-sections")
            SET(XPCOM_CXX_FLAGS "${XPCOM_CXX_FLAGS} -fdata-sections -ffunction-sections")
