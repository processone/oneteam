  ...
  this._handleEvent = function(event,arg) {
    ...
    for (var i=0;i<this._events[event].length; i++) {
      if (this._events[event][i]) {
        ...
        } catch (e) { alert(e.name+": "+e);this.oDbg.log(e.name+": "+ e.message); }
      }
    }
  };
...
function JSJaCStreamBind() {
  ...
  var eBind = iq.getDoc().createElementNS(ns, "bind");
  eBind.appendChild(iq.getDoc().createElementNS(ns, "resource"))
...
function JSJaCXMPPSess(iq) {
  ...
  var eSess = iq.getDoc().createElementNS(ns, "session");
...
function JSJaCError(code,type,condition) {
  ...
  xmldoc.documentElement.appendChild(
    xmldoc.createElementNS("urn:ietf:params:xml:ns:xmpp-stanzas", condition));
