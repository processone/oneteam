...
function build () {
    ...
    rm  ../build/$2/*.xulapp \
      ../build/$2/*-partial.mar \
      ../build/$2/*.xpi 2>/dev/null
    cp *.xulapp *.mar *.xpi update.rdf ../build/$2/

    perl -e '
        @files = map { $_->[1] }
          sort { $a->[0] cmp $b->[0] }
          map { my $x = $_; $x =~ s/.*?(\d+(?:\.\d+)+).*/$1/; $x =~ s/(\d+)/sprintf "%010d", $1/ge; [$x, $_] }
          glob("../build/$ARGV[0]/*.mar");
        ...
        for (@files[0..$#files-2]) {
            ...
            system("tools/make-partial-mar.pl", $_, $files[/-mac/ ? -2 : -1], $patch, "mars-info.txt",
    ...
    if test "$2" != "release"; then
        rsync -a --delete ../build/$2/ s2:download.process-one.net/$2/
